...
export default function AdminView() {
	const { userProfile, signOut, setIgnoreAuthChanges, setOriginalSession } =
		useAuth();
	...
	const [showEditDoctorUnassignConfirm, setShowEditDoctorUnassignConfirm] =
		useState(false);
	...
	const [showClinicFilterDropdown, setShowClinicFilterDropdown] =
		useState(false);
	...
	const [showStaffClinicSuggestions, setShowStaffClinicSuggestions] =
		useState(false);
	const [staffClinicSearchFocused, setStaffClinicSearchFocused] =
		useState(false);
		...
		try {
			...
			if (authUuid) {
				try {
					...
					if (!supabaseAdmin) {
						console.warn(
							"Supabase Admin client not initialized. Service role key may be missing."
						);
		...
		try {
			...
			if (editingUserData.role === "PATIENT" && editingUserData.patientData) {
				...
				const patientUpdateData = {
					...
					emergencyContact: toNullIfEmpty(
						editingUserData.patientData.emergencyContact
					),
					emergencyContactPhone: toNullIfEmpty(
						editingUserData.patientData.emergencyContactPhone
					),
	...
	const handleUpdateClinicInterval = async (clinicId) => {
		if (
			!editingInterval ||
			isNaN(editingInterval) ||
			parseInt(editingInterval) <= 0
		) {
			setToast({
		...
		try {
			setLoading(true);
			await clinicAPI.update(clinicId, {
				apptIntervalMin: parseInt(editingInterval),
			});
			setToast({
	...
	const formatOperatingHours = (clinic) => {
		...
		if (clinic.monFriAmStart || clinic.monFriPmStart) {
			...
			if (clinic.monFriAmStart && clinic.monFriAmEnd) {
				monFriHours.push(
					`${formatTime(clinic.monFriAmStart)} - ${formatTime(clinic.monFriAmEnd)}`
				);
			}
			if (clinic.monFriPmStart && clinic.monFriPmEnd) {
				monFriHours.push(
					`${formatTime(clinic.monFriPmStart)} - ${formatTime(clinic.monFriPmEnd)}`
				);
			}
		...
		if (clinic.satAmStart && clinic.satAmEnd) {
			hours.push({
				day: "Saturday",
				times: `${formatTime(clinic.satAmStart)} - ${formatTime(clinic.satAmEnd)}`,
			});
		}
		...
		if (clinic.sunAmStart && clinic.sunAmEnd) {
			hours.push({
				day: "Sunday",
				times: `${formatTime(clinic.sunAmStart)} - ${formatTime(clinic.sunAmEnd)}`,
			});
		}
		...
		if (clinic.phAmStart && clinic.phAmEnd) {
			hours.push({
				day: "Public Holiday",
				times: `${formatTime(clinic.phAmStart)} - ${formatTime(clinic.phAmEnd)}`,
			});
		}
	...
	const handleUnassignDoctorFromClinic = async () => {
		...
		try {
			...
			setClinicDoctorsMap((prev) => ({
				...prev,
				[clinicToUnassignFrom]: doctors || [],
			}));
	...
	const handleEditDoctorShiftsInClinic = (doctor, clinicId) => {
		const shiftDaysArray = Array.isArray(doctor.shiftDays)
			? doctor.shiftDays
			: doctor.shiftDays
			? [doctor.shiftDays]
			: [];
	...
	const handleSaveDoctorShiftsInClinic = async (doctorId, clinicId) => {
		try {
			...
			await doctorAPI.update(doctorId, {
				shiftDays:
					editingDoctorShifts.length > 0 ? editingDoctorShifts.sort() : null,
			});
			setToast({
	...
	const handleAssignDoctorToClinic = async (doctorId, clinicId) => {
		try {
			...
			await doctorAPI.update(doctorId, {
				...
				shiftDays:
					assigningDoctorShifts.length > 0 ? assigningDoctorShifts.sort() : null,
			});
			setToast({
	...
	const handleCreateDoctor = async (e) => {
		...
		try {
			...
			setNewDoctorForm({
				fname: "",
				lname: "",
				assignedClinic: selectedClinicForDoctors.id,
			});
			await openDoctorModal(selectedClinicForDoctors);
		} catch (err) {
			setToast({
	...
	const fetchDoctorsForView = async () => {
		try {
			...
			data?.forEach((doctor) => {
				if (
					doctor.shiftDays &&
					doctor.shiftDays.length > 0 &&
					(!editingDoctor || doctor.id !== editingDoctor.id)
				) {
					// Only count filled days for doctors in the same clinic
					if (
						!selectedClinicFilter ||
						doctor.assignedClinic === selectedClinicFilter
					) {
		...
		doctors.forEach((d) => {
			if (d.shiftDays && d.id !== doctor.id) {
				...
				if (!doctorClinic || d.assignedClinic === doctorClinic) {
					const days = Array.isArray(d.shiftDays)
						? d.shiftDays
						: d.shiftDays
						? [d.shiftDays]
						: [];
	...
	const updateDoctor = async () => {
		...
		try {
			...
			await doctorAPI.update(editingDoctor.id, {
				...
				shiftDays:
					editingDoctor.shiftDays.length > 0 ? editingDoctor.shiftDays : null,
			});
			setToast({
	...
	useEffect(() => {
		const handleClickOutside = (event) => {
			...
			const isInsideDropdown =
				target.closest(".searchable-dropdown") !== null ||
	...
	const handleGenerateReport = async () => {
		try {
			...
			const blob = await reportAPI.getSystemUsageReport(
			...
			// Generate filename
			const startStr = reportStartDate
				? reportStartDate.replace(/-/g, "")
				: "start";
			...
			<div className="admin-layout">
				<div className="admin-main">
									...
									<div className="chart-container">
										{Object.keys(dashboardStats.appointmentsByStatus).length > 0 ? (
											<ResponsiveContainer width="100%" height={300}>
												<PieChart>
													<Pie
														data={Object.entries(dashboardStats.appointmentsByStatus).map(
															([status, count]) => ({
																name: status,
																value: count,
															})
													...
													>
														{Object.keys(dashboardStats.appointmentsByStatus).map(
															(status, index) => {
																const colors = [
																	"#4ECDC4",
																	"#95E1D3",
													...
													<Legend
														...
														height={36}
														formatter={(value) =>
															`${value}: ${dashboardStats.appointmentsByStatus[value] || 0}`
														}
													/>
												</PieChart>
											</ResponsiveContainer>
												...
												<div className="chart-stats">
													<div className="chart-stat-item">
														...
														<span className="stat-value">
															{dashboardStats.appointmentsOverTime.reduce(
																(sum, d) => sum + d.count,
																0
															)}
														</span>
														...
														<span className="stat-value">
															{(
																dashboardStats.appointmentsOverTime.reduce(
																	(sum, d) => sum + d.count,
																	0
																) / dashboardStats.appointmentsOverTime.length
															).toFixed(1)}
														</span>
													</div>
												</div>
						...
						<>
							...
							{showCreateStaff && (
								<div className="section-card">
									...
									<p className="form-description">
										Create a new staff member account. Staff members are assigned to a
										specific clinic.
									</p>
									<form onSubmit={handleCreateStaff} className="create-form">
										...
										<div className="form-group">
											...
											<div className="clinic-autocomplete-wrapper">
												...
												/>
												{showStaffClinicSuggestions &&
													staffClinicSearchFocused &&
													filteredStaffClinics.length > 0 && (
												...
												{showStaffClinicSuggestions &&
													...
													staffClinicSearch && (
														<div className="clinic-suggestions-dropdown">
															<div className="clinic-suggestion-no-results">
																No clinics found
															</div>
														</div>
													)}
							...
							{showCreatePatient && (
								<div className="section-card">
									...
									<p className="form-description">
										Create a new patient account. Patients can book appointments and view
									...
									<form onSubmit={handleCreatePatient} className="create-form">
										...
										<div className="form-row">
											<div className="form-group">
												<label htmlFor="patient-emergency-contact">
											...
											<div className="form-group">
												<label htmlFor="patient-emergency-phone">
																		...
																		{isEditing ? (
																			<div className="user-edit-form">
																						...
																						<div className="form-row">
																							<div className="form-group">
																								...
																								<input
																									type="text"
																									value={
																										editingUserData?.patientData?.patientIc ||
																										patient.patientIc ||
																										""
																									}
																									onChange={(e) =>
																							...
																							<div className="form-group">
																								...
																								<input
																									type="date"
																									value={
																										editingUserData?.patientData?.dateOfBirth ||
																										(patient.dateOfBirth
																							...
																							<div className="form-group">
																								...
																								<select
																									value={
																										editingUserData?.patientData?.gender ||
																										patient.gender ||
																										"Male"
																									}
																									onChange={(e) =>
																					...
																					<>
																						...
																						<div className="form-section">
																							...
																							<div className="form-group">
																								...
																								<select
																									value={
																										editingUserData?.patientData?.bloodType ||
																										patient.bloodType ||
																										""
																									}
																									onChange={(e) =>
																							...
																							<div className="form-group">
																								...
																								<textarea
																									value={
																										editingUserData?.patientData?.allergies ||
																										patient.allergies ||
																										""
																									}
																									onChange={(e) =>
																							...
																							<div className="form-group">
																								...
																								<textarea
																									value={
																										editingUserData?.patientData?.medicalHistory ||
																				...
																				{user.role === "STAFF" && staffMember && (
																					<div className="form-section">
																						...
																						<div className="form-group">
																							...
																							<select
																								value={
																									editingUserData?.staffData?.clinicId ||
																				...
																				<div className="form-actions">
																					<button
																						onClick={handleResetPassword.bind(null, user.email)}
																							...
																							<div className="detail-item">
																								...
																								<span className="detail-value">
																									{staffMember.clinicName ||
																										(staffMember.clinicId
																											? `Clinic ID: ${staffMember.clinicId}`
																											: "N/A")}
																								</span>
																							</div>
																						</div>
					...
					{currentView === "clinics" &&
						(() => {
							// Filter clinics based on search
							const filteredClinics = clinicSearch
								? clinics.filter(
										(c) =>
												...
												<div className="clinics-cards-container">
													{paginatedClinics.map((clinic) => {
														...
														return (
															<div
																key={clinic.id}
																className={`clinic-card ${isExpanded ? "expanded" : ""}`}
															>
																<div
																...
																>
																	<div className="clinic-card-main-info">
																		...
																		<div className="clinic-card-location">
																			{clinic.address || "N/A"} • {clinic.region || "N/A"} •{" "}
																			{clinic.area || "N/A"}
																		</div>
																	</div>
																	...
																	<div className="clinic-card-expanded-content">
																				...
																				<div className="clinic-doctors-edit">
																					...
																					<div className="assigned-doctors-section">
																						...
																						{clinicDoctorsList.length === 0 ? (
																							<p className="clinic-detail-empty">
																								No doctors assigned
																							</p>
																						) : (
																							<div className="clinic-doctors-list">
																								{clinicDoctorsList.map((doctor) => {
																									const doctorShiftDays = Array.isArray(doctor.shiftDays)
																										? doctor.shiftDays
																										: doctor.shiftDays
																										? [doctor.shiftDays]
																										: [];
																									const isEditingShifts =
																										editingDoctorInClinic === doctor.id;

																									return (
																										<div
																											key={doctor.id}
																											className="clinic-doctor-item-edit"
																										>
																												...
																												{!isEditingShifts ? (
																													<>
																														{doctorShiftDays.length > 0 && (
																															<div className="shift-days-display">
																																{[1, 2, 3, 4, 5, 6, 7].map((day) => (
																																	<span
																																		key={day}
																																		className={`shift-day-badge ${
																																			doctorShiftDays.includes(day)
																																				? "active"
																																				: "inactive"
																																		}`}
																														...
																														<div className="doctor-item-actions">
																															<button
																																onClick={(e) => {
																																	e.stopPropagation();
																																	handleEditDoctorShiftsInClinic(
																																		doctor,
																																		clinic.id
																																	);
																																}}
																													...
																													<div className="doctor-shifts-edit-container">
																														<div className="shift-days-selector">
																															{[1, 2, 3, 4, 5, 6, 7].map((day) => {
																																const isSelected =
																																	editingDoctorShifts.includes(day);
																																...
																																const isFilled = clinicDoctorsList.some((d) => {
																																	...
																																	const dShiftDays = Array.isArray(d.shiftDays)
																																		? d.shiftDays
																																		: d.shiftDays
																																		? [d.shiftDays]
																																		: [];
																														...
																														<div className="doctor-shift-actions">
																															<button
																																onClick={(e) => {
																																	e.stopPropagation();
																																	handleSaveDoctorShiftsInClinic(
																																		doctor.id,
																																		clinic.id
																																	);
																																}}
																													...
																													{!isAssigning ? (
																														...
																														<div className="doctor-shifts-edit-container">
																															<div className="shift-days-selector">
																																{[1, 2, 3, 4, 5, 6, 7].map((day) => {
																																	const isSelected =
																																		assigningDoctorShifts.includes(day);
																																	// Check if this day is already taken by another doctor in the same clinic
																																	const isFilled = clinicDoctorsList.some(
																																		(d) => {
																																			const dShiftDays = Array.isArray(d.shiftDays)
																																				? d.shiftDays
																																				: d.shiftDays
																		...
																		<div className="clinic-detail-section">
																			<div className="clinic-detail-header">
																				...
																				<button
																					onClick={(e) => {
																						...
																						setEditingInterval(
																							clinic.apptIntervalMin?.toString() || "15"
																						);
																					}}
																					className="btn btn-secondary btn-sm"
																			...
																			{editingClinicId === clinic.id ? (
																				...
																				<p className="clinic-detail-value">
																					{clinic.apptIntervalMin || 15} minutes
																				</p>
																			)}
																		</div>
																	</div>
												...
												{totalPages > 1 && (
													<div className="pagination-controls">
														...
														<span className="pagination-info">
															Page {clinicPage} of {totalPages}({startIndex + 1}-
															{Math.min(endIndex, filteredClinics.length)} of{" "}
															{filteredClinics.length})
														</span>
														<button
															onClick={() =>
																setClinicPage((prev) => Math.min(totalPages, prev + 1))
															}
											...
											<div className="doctor-search-wrapper">
												...
												<div className="searchable-dropdown">
													...
													{showDoctorSearchDropdown && doctorNameSearch && (
														<div
															className="dropdown-menu doctor-search-dropdown"
															onClick={(e) => e.stopPropagation()}
														>
															{getFilteredDoctorsForSearch().map((doctor) => (
																...
																>
																	<span className="doctor-suggestion-name">
																		Dr. {doctor.fname} {doctor.lname}
																	</span>
																	{doctor.assignedClinic && (
																		<span className="doctor-suggestion-clinic">
																			{clinics.find((c) => c.id === doctor.assignedClinic)?.name ||
																				`Clinic ${doctor.assignedClinic}`}
																		</span>
																	)}
																</div>
															))}
												...
												<div className="searchable-dropdown">
													...
													{showClinicFilterDropdown && (
														...
														>
															...
															{clinicFilterSearch &&
																getFilteredClinicsForFilter().length === 0 && (
																	<div
																		className="dropdown-item"
																		style={{ color: "#999", cursor: "default" }}
																	>
											...
											<button
												onClick={() => {
													...
													doctors.forEach((doctor) => {
														if (doctor.shiftDays && doctor.shiftDays.length > 0) {
															...
															if (
																...
																const days = Array.isArray(doctor.shiftDays)
																	? doctor.shiftDays
																	: doctor.shiftDays
																	? [doctor.shiftDays]
																	: [];
													...
													setNewDoctor({
														fname: "",
														lname: "",
														assignedClinic: selectedClinicFilter || null,
														shiftDays: [],
													});
										...
										<div className="section-card unassigned-doctors-section">
											...
											<div className="doctors-grid compact-grid">
												{unassignedDoctors.slice(0, 6).map((doctor) => (
													<div
														key={doctor.id}
														className="doctor-card unassigned-doctor-card"
													>
														...
														<div className="doctor-actions">
															<button
																onClick={() => {
																	setEditingDoctor({
																		...doctor,
																		shiftDays: Array.isArray(doctor.shiftDays)
																			? doctor.shiftDays
																			: doctor.shiftDays
																			? [doctor.shiftDays]
																			: [],
												...
												{unassignedDoctors.length > 6 && (
													<div className="view-more-hint">
														<p>
															+ {unassignedDoctors.length - 6} more unassigned doctor
															{unassignedDoctors.length - 6 !== 1 ? "s" : ""}
														</p>
													</div>
												)}
											</div>
									...
									<div className="section-card">
										<div className="section-header">
											<h2>
												{selectedClinicFilter
													? `Doctors at ${
															clinics.find((c) => c.id === selectedClinicFilter)?.name ||
															"Selected Clinic"
											...
											<>
															...
															<div key={doctor.id} className="doctor-card">
																...
																<div className="doctor-details">
																	<div className="detail-item">
																		...
																		<span className="detail-value">
																			{doctor.assignedClinic
																				? clinics.find((c) => c.id === doctor.assignedClinic)
																						?.name || `Clinic ID: ${doctor.assignedClinic}`
																				: "Unassigned"}
																		</span>
																	</div>
																	{doctorShiftDays.length > 0 ? (
																			...
																			<div className="shift-days-display">
																				{[1, 2, 3, 4, 5, 6, 7].map((day) => (
																					<span
																						key={day}
																						className={`shift-day-badge ${
																							doctorShiftDays.includes(day) ? "active" : "inactive"
																						}`}
																						title={getDayFullName(day)}
																					>
																						{getDayLabel(day)}
												...
												{totalPages > 1 && (
													<div className="pagination-controls">
														...
														<span className="pagination-info">
															Page {doctorPage} of {totalPages}({startIndex + 1}-
															{Math.min(endIndex, doctors.length)} of {doctors.length})
														</span>
														<button
															onClick={() =>
																setDoctorPage((prev) => Math.min(totalPages, prev + 1))
															}
												...
												<div className="modal-body">
													<div className="form-group">
														...
														<input
															...
															onChange={(e) =>
																setEditingDoctor({ ...editingDoctor, fname: e.target.value })
															}
															className="input-sm"
													...
													<div className="form-group">
														...
														<input
															...
															onChange={(e) =>
																setEditingDoctor({ ...editingDoctor, lname: e.target.value })
															}
															className="input-sm"
													...
													<div className="form-group">
														...
														<p className="form-hint">
															Select the days this doctor will work. Greyed out days are
															already assigned to other doctors in the same clinic. Leave empty
															if no shifts assigned.
														</p>
														<div className="shift-days-selector">
															{[1, 2, 3, 4, 5, 6, 7].map((day) => {
																const isFilled = filledDays.has(day);
																const shiftDaysArray = Array.isArray(editingDoctor.shiftDays)
																	? editingDoctor.shiftDays
																	: editingDoctor.shiftDays
																	? [editingDoctor.shiftDays]
																	: [];
																...
																return (
																	<button
																		...
																		title={
																			isFilled
																				? `${getDayFullName(day)} is already assigned`
																				: `${getDayFullName(day)} - Click to toggle`
																		}
																	>
														...
														{editingDoctor.shiftDays &&
															editingDoctor.shiftDays.length > 0 && (
																<div className="selected-days-info">
																	<p>
																		Selected:{" "}
																		{Array.isArray(editingDoctor.shiftDays)
																			? editingDoctor.shiftDays
																					.map((d) => getDayFullName(d))
																					.join(", ")
																			: getDayFullName(editingDoctor.shiftDays)}
																	</p>
																</div>
															)}
													</div>
													<div className="modal-actions">
														...
														<button
															...
															className="btn btn-primary"
															disabled={
																loading || !editingDoctor.fname || !editingDoctor.lname
															}
														>
												...
												<div className="modal-body">
													<div className="form-group">
														...
														<input
															...
															onChange={(e) =>
																setNewDoctor({ ...newDoctor, fname: e.target.value })
															}
															className="input-sm"
															placeholder="Enter first name"
														/>
													...
													<div className="form-group">
														...
														<input
															...
															onChange={(e) =>
																setNewDoctor({ ...newDoctor, lname: e.target.value })
															}
															className="input-sm"
															placeholder="Enter last name"
														/>
													...
													<div className="form-group">
														...
														{newDoctor.shiftDays.length > 0 && (
															<div className="selected-days-info">
																<p>
																	Selected:{" "}
																	{newDoctor.shiftDays.map((d) => getDayFullName(d)).join(", ")}
																</p>
															</div>
														)}
													</div>
						...
						<>
							<div className="page-header">
								...
								<p className="subtitle">
									Generate comprehensive system-wide usage reports
								</p>
							</div>
							...
							<div className="section-card">
								...
								<p className="form-description">
									Generate a comprehensive PDF report with system-wide statistics
									including appointments, cancellations, patients seen, average waiting
									time, and no-show rates. Default period is from the start of the
								...
								<div className="report-form">
									<div className="form-row">
										<div className="form-group">
											...
											<small className="form-hint">
												If empty, defaults to start of current month
											</small>
										</div>
										<div className="form-group">
									...
									<div className="report-metrics-info">
										...
										<ul className="metrics-list">
											<li>
												📊 <strong>Total Appointments Booked</strong> - All appointments
												excluding cancelled ones
											</li>
											<li>
												❌ <strong>Total Cancellations</strong> - All cancelled appointments
											</li>
											<li>
												👥 <strong>Patients Seen</strong> - All queue logs with completed
												status
											</li>
											<li>
												⏱️ <strong>Average Waiting Time</strong> - Mean time from
												appointment start to check-in
											</li>
											<li>
												🚫 <strong>No-Show Rate</strong> - Ratio of appointments without
												queue logs
											</li>
										</ul>
									</div>
							...
							<div className="section-card">
								...
								<div className="settings-form">
									<div className="form-group">
										...
										<input
											type="text"
											value={`${userProfile?.fname} ${userProfile?.lname}`}
											disabled
										/>
									</div>
									<div className="form-group">
						...
						<div className="modal-body">
							...
							>
								<h3 style={{ marginTop: 0, marginBottom: "1rem" }}>
									Assigned Doctors ({clinicDoctors.length})
								</h3>
							...
							>
								...
								{allDoctors.filter(
									(d) =>
										!d.assignedClinic || d.assignedClinic !== selectedClinicForDoctors.id
								).length === 0 ? (
									...
									<div className="doctors-list">
										{allDoctors
											.filter(
												(d) =>
													!d.assignedClinic ||
													d.assignedClinic !== selectedClinicForDoctors.id
											)
											.map((doctor) => (
												<div key={doctor.id} className="doctor-item">
													<div className="doctor-info">
														...
														{doctor.assignedClinic && (
															<span className="assigned-to">
																Currently at Clinic {doctor.assignedClinic}
															</span>
														)}
													</div>
													<button
														onClick={() =>
															handleAssignDoctor(doctor.id, selectedClinicForDoctors.id)
														}
														className="btn btn-success btn-sm"
														disabled={loading}
													>
					...
					>
						...
						<div className="modal-body">
							<p className="confirm-message">
								...
								<strong>
									Dr. {doctorToUnassign.fname} {doctorToUnassign.lname}
								</strong>{" "}
								from this clinic?
							</p>
							<p className="confirm-warning">
								This will remove the doctor from the clinic. You can reassign them later
								if needed.
							</p>
							<div className="modal-actions">
								<button
						...
						>
							...
							<div className="modal-body">
								<p className="confirm-message">
									Are you sure you want to unassign{" "}
									<strong>
										Dr. {editingDoctor.fname} {editingDoctor.lname}
									</strong>{" "}
									from{" "}
									{clinics.find((c) => c.id === editingDoctor.assignedClinic)?.name ||
										"the clinic"}
									?
								</p>
								<p className="confirm-warning">
									This will remove the doctor from the clinic and clear their shift days.
